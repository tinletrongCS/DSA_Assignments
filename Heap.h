...
////////////////////////////////////////////
/*
* Sinh viên thực hiện : LÊ TRỌNG TÍN
* ID : 2313452
...
void Heap<T>::remove(T item, void (*removeItemData)(T)) {
    ...
    if (removeItemData) {
        removeItemData(this->elements[foundIdx]);
    }
    this->elements[foundIdx] = this->elements[count - 1];
    this->count--;
    reheapDown(foundIdx);
}
...
void Heap<T>::reheapUp(int position) {
    // tìm index của node cha
    int parent = (position - 1) / 2;
    if (position > 0 && this->compare(elements[parent], elements[position]) > 0)
    {
...
void Heap<T>::reheapDown(int position) {
    ...
    int left = position * 2 + 1, right = position * 2 + 2;
    if (left < this->size() && this->compare(elements[left], elements[largest]) < 0)
    {
        largest = left;
    }
    if (right < this->size() && this->compare(elements[right], elements[largest]) < 0)
    {
        largest = right;
    }
    if (largest != position)
