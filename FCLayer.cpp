                 ...
                 string sName)
{
  ...
  try
  {
    ...
    while (getline(param_stream, param_slist[idx], delimiter))
    {
      ...
      idx++;
      if (idx >= 3)
        break; // just use the first three elements
    }
    if (idx == 2)
    {
...
xt::xarray<double> FCLayer::backward(xt::xarray<double> DY)
{
  ...
  double_tensor dW_batch = xt::zeros<double>({m_aWeights.shape()[0], m_aWeights.shape()[1]});
  dW_batch = outer_stack(DY, m_aCached_X);
  m_aGrad_W = xt::sum(dW_batch, {0});
  double_tensor DX = xt::linalg::dot(DY, m_aWeights);
  return DX;
}
