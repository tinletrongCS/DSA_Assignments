...
FCLayer::FCLayer(string sParams, string filename_w, string filename_b,
                 string sName) {
  // update name
  if (trim(sName).size() != 0)
    this->m_sName = sName;
  else
    m_sName = "FC_" + to_string(++m_unLayer_idx);
  ...
  try {
    ...
    if (idx == 2) {
      ...
      throw std::runtime_error(
          "FC's parameters: must specify at least Nin and Nout");
    }
    //
    ...
    if (weight_file_invalid) {
      ...
      string message = fmt::format(
          "{:s}: not exist; so initialize weights with random numbers",
          filename_w);
      cout << message << endl;
      ...
      bool valid = (W.dimension() == 2) && (W.shape()[0] == m_nNout) &&
                   (W.shape()[1] == m_nNin);
      if (!valid) {
        throw std::runtime_error(
            "FC::Weights: shape from data file is not the same with the "
            "specification");
      }
      this->m_aWeights = W;
      this->m_aGrad_W =
          xt::zeros<double>({m_nNout, m_nNin});  // initialize gradW
    }
    if (bias_file_invalid) {
      // Bias file is not specified correctly => initialize with 0
      string message = fmt::format(
          "{:s}: not exist; so initialize biases with 0", filename_b);
      cout << message << endl;
      ...
      if (m_bUse_Bias) {
        ...
        if (!valid)
          throw std::runtime_error(
              "FC::Bias: shape from data is not the same with the "
              "specification");
    ...
    string message = fmt::format(string("\tsParams: \"{:s}\";\r\n") +
                                     string("\tfilename_w: \"{:s}\";\r\n") +
                                     string("\tfilename_b: \"{:s}\";\r\n") +
...
{
    if (this->has_learnable_param())
    {
        this->m_aCached_X = X;
    }
    xt::xarray<double> result = xt::linalg::tensordot(X, this->m_aWeights, { 1 }, { 0 });
    if (this->m_bUse_Bias)
    {
        result += this->m_aBias;
    }
...
xt::xarray<double> FCLayer::backward(xt::xarray<double> DY)
{
    ...
    this->m_aGrad_W = xt::linalg::tensordot(DY, m_aCached_X, { 0 }, { 0 });
    // tính delta b nếu có sử dụng bias
    if (this->m_bUse_Bias)
    {
        this->m_aGrad_b = xt::sum(DY, { 0 });
    }

    xt::xarray<double> DX = xt::linalg::tensordot(DY, this->m_aWeights);
    return DX;
}
...
string FCLayer::get_desc() {
  string desc = fmt::format("{:<10s}, {:<15s}: {:<4d}, {:<4d}, {:<4d}", "FC",
                            this->getname(), this->m_nNin, this->m_nNout,
                            this->m_bUse_Bias);
  return desc;
}
 ...
 *  + model_path: folder that contains the models' data files: arch.txt and
 * others.
 *  + layer_name: name of FC layer; maybe an empty ("")
 * PRE-CONDITION:
 *  + if param "layer_name" is empty then m_sName in FCLayer MUST BE assigned in
 * advance. POST-CONDITION:
...
void FCLayer::load(string model_path, string layer_name) {
  ...
  try {
    if (fs::exists(filename_w)) {
      ...
      string message =
          fmt::format("{:s}: weight-file does not exist.", filename_w);
