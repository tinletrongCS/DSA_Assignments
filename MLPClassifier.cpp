...
double_tensor MLPClassifier::predict(DataLoader<double, double> *pLoader,
                                     bool make_decision)
{
  cout << "running prediction for a batch-data" << endl;
  bool old_mode = this->m_trainable;
  this->set_working_mode(false);

  double_tensor results;

  // biến dùng cho đánh dấu batch dữ liệu đầu tiên
...
void MLPClassifier::backward()
{
  ...
  // typename DLinkedList<ILayer*>::BWDIterator
...
bool MLPClassifier::save(string model_path)
{
  ...
  {
    string message =
        fmt::format("MLPClassifier::save: failed; model_path={:s}", model_path);
    cerr << message << endl;
    cerr << e.what() << endl;
    return false;
  }
...
bool MLPClassifier::load(string model_path, bool use_name_in_file)
{
  try
  {
    ...
    {
      string message =
          fmt::format("{:s}: can not open for reading.", arch_file);
      cerr << message << endl;
      return false;
    }
    ...
    while (getline(datastream, line))
    {
      ...
      getline(partstream, layer_type, delimiter); // type: maybe an empty
                                                  // string
      getline(partstream, layer_name, delimiter); // name: maybe an empty
                                                  // string
      ...
      if (layer_type.compare("Softmax") == 0)
      {
        ...
        {
          string message_1 = fmt::format(
              "Can not read axis of Softmax from: {:s}", trim(second));
